...
class MeanField(object):
    ...
    def __init__(self, h_dim, x_dim, hid_dim):
        ...
        self.cov_chols_h_nn = self.cov_chols_h_nn_fn()

    def get_params(self):

        means_h_params = lasagne.layers.get_all_params(self.means_h_nn)
        cov_chols_h_params = lasagne.layers.get_all_params(self.cov_chols_h_nn)

        return means_h_params + cov_chols_h_params

    def get_param_values(self):

        means_h_params = lasagne.layers.get_all_param_values(self.means_h_nn)
        cov_chols_h_params = lasagne.layers.get_all_param_values(self.cov_chols_h_nn)

        return [means_h_params, cov_chols_h_params]

    def set_param_values(self, param_values):

        [means_h_params, cov_chols_h_params] = param_values

        lasagne.layers.set_all_param_values(self.means_h_nn, means_h_params)
        lasagne.layers.set_all_param_values(self.cov_chols_h_nn, cov_chols_h_params)
    ...
    def get_samples_latents(self, X, det=False):
        ...
        cov_chols = lasagne.layers.get_output(self.cov_chols_h_nn, X, deterministic=det)

        samples = means + (cov_chols * e)
    ...
    def entropies_latents(self, H, X, det=False):
        ...
        entropies = -utilities.densities.log_diagonal_gaussian(H, means, cov_chols)
